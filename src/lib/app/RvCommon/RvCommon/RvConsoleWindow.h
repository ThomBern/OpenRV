...
public:
    ...
    {
      public:
        ConsoleBuf(RvConsoleWindow *w) : m_console(w) {}
        int sync();
        int overflow( int ch );
    ...
    // To be called in place of processTextBuffer (see below) prior to exiting 
    // the process exit(-1). This will prevent some funny business in the exit()
    // function such as trying to start QtTimers in ConsoleBuf::sync() for example
